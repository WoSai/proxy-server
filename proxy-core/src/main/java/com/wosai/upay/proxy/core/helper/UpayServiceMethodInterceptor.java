package com.wosai.upay.proxy.core.helper;
...
public class UpayServiceMethodInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        } catch (ProxyCoreBizException ex) {
            logger.warn(String.format("biz error in proxy-core service %s", ex.getCode()), ex);
            throw ex;

        } catch (ProxyCoreSystemException ex) {
            logger.error(String.format("sys error in proxy-core service %s", ex.getCode()), ex);
            throw ex;
            
        } catch (Throwable ex) {
            logger.error("unknown critical error in proxy-core service", ex);
            throw ex;
