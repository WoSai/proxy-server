package com.wosai.upay.proxy.auto.helper;
...
public class UpayServiceMethodInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        } catch (ProxyAutoBizException ex) {
            logger.warn(String.format("biz error in proxy-auto service %s", ex.getCode()), ex);
            throw ex;

        } catch (ProxyAutoSystemException ex) {
            logger.error(String.format("sys error in proxy-auto service %s", ex.getCode()), ex);
            throw ex;
            
        } catch (Throwable ex) {
            logger.error("unknown critical error in proxy-auto service", ex);
            throw ex;
